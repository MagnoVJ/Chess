...
bool Player::makeMovement(Board& board, Piece piece, char colActualPos, int rowActualPos, char colTargetPos, int rowTargetPos){

	if(rowTargetPos < 1 || rowTargetPos > 8){
		std::cout << "ERRO::Player::makeMovement:: row not valid" << std::endl;
		return false;
	}
	...
	if(!(colTargetPos == 'A' || colTargetPos == 'B' || colTargetPos == 'C' || colTargetPos == 'D' || colTargetPos == 'E' || colTargetPos == 'F' || colTargetPos == 'G' || colTargetPos == 'H' || colTargetPos == 'a' || colTargetPos == 'b' || colTargetPos == 'c' || colTargetPos == 'd' || colTargetPos == 'e' || colTargetPos == 'f' || colTargetPos == 'g' || colTargetPos == 'h')){
		std::cout << "ERRO::Player::makeMovement:: col not valid" << std::endl;
		return false;
	}

	if(piece.canMakeaMovementPlayer(colActualPos, rowActualPos, colTargetPos, rowTargetPos, piece.type)){
		...
		if(!(piece.type == KNIGHT || piece.type == KING))
			if(!verifPieceOntheWay(board, piece, colActualPos, rowActualPos, colTargetPos, rowTargetPos)){
				std::cout << std::endl << "ERROR::Player::verifPieceIntheWay:: ha peca no caminho, movimento pribido" << std::endl;
				return false;
			}
	...
	std::cout << "ERRO::Player::makeMovement:: nao foi possivel faser o movimento" << std::endl;
	return false;

}

bool Player::verifPieceOntheWay(const Board& board, Piece piece, char colActualPos, int rowActualPos, char colTargetPos, int rowTargetPos){
...
bool Player::verifCastling(const Board& board, char colActualPos, int rowActualPos, char colTargetPos, int rowTargetPos, bool& castlingLeft, bool& castlingRight) {
		...
		if (king != nullptr && rock != nullptr) {
			...
			if (king->neverMoved == true && rock->neverMoved == true) {
				...
				if (board.getPiece(colActualPos - 1, rowActualPos) == nullptr &&
					...
					std::cout << std::endl << "ERROR::Rock nao e permitido pois existe peca entre o rei e a torre" << std::endl;
					return false;
				}

			}
			else {
				std::cout << std::endl << "ERROR::Rock nao e permitido pois o rei ou a torre ja foram movidos anteriormente" << std::endl;
				return false;
			}

		}
		else {
			std::cout << std::endl << "ERROR::Rock nao e permitido pois o rei ou a torre nao se encontram na posicao correta" << std::endl;
			return false;
		}
...
bool Player::selecPieceVerif(Board board, char col, int row){

	int x, y;
	
	if(row < 1 || row > 8){
		std::cout << "ERRO::Player::selecPieceVerif:: row not valid" << std::endl;
		return false;
	}
	...
	if(col == 'A' || col == 'B' || col == 'C' ||
	   ...
	   x = col - 97;
		...
		std::cout << "ERRO::Player::selecPieceVerif:: col not valid" << std::endl;
		return false;
	}

	if(board.pieces[x + y * 8] != nullptr)
		if(board.pieces[x + y * 8]->color == WHITE)
			return true;
...
bool Player::selecDestiVerif(Board board, char col, int row){
...
void Player::verifEnpassant(char colActualPos, int rowActualPos, char colTargetPos, int rowTargetPos, bool& movimentoEnPassant) {

	std::cout << std::endl << "**Verificacao En passant**" << std::endl;

	//Verifica En passant
	if (rowActualPos == 5) { //Verifica se o peao está na 5 casa do tabuleiro
		...
		std::cout << std::endl << "->Não e um movimento En passant pois o peao nao se encontra na casa 5" << std::endl;
